...
_modbus_cli_complete() {
  ...
  local cmds="init-config validate-config test-connection status read write points read-point write-point watch-point identify watch completions version"
...
_arguments \
  '1:command:(init-config validate-config test-connection status read write points read-point write-point watch-point identify watch completions version)' \
  '*::arg:->args'`)
		return err
	default:
