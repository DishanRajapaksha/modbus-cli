...
_modbus_cli_complete() {
  ...
  local cmds="init-config validate-config test-connection status read write identify watch completions version"
...
_arguments \
  '1:command:(init-config validate-config test-connection status read write identify watch completions version)' \
  '*::arg:->args'`)
		return err
	default:
